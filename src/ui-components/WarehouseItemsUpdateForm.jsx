...
export default function WarehouseItemsUpdateForm(props) {
  const {
    id: idProp,
    warehouseItems,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = warehouseItemsRecord
      ? { ...initialValues, ...warehouseItemsRecord }
      : initialValues;
    ...
    setWarehouse(cleanValues.Warehouse);
    setLocationInWarehouse(cleanValues.LocationInWarehouse);
    setQuantity(cleanValues.Quantity);
    setErrors({});
  };
  const [warehouseItemsRecord, setWarehouseItemsRecord] =
    React.useState(warehouseItems);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(WarehouseItems, idProp)
        : warehouseItems;
      setWarehouseItemsRecord(record);
    };
    queryData();
  }, [idProp, warehouseItems]);
  React.useEffect(resetStateValues, [warehouseItemsRecord]);
  const validations = {
    Category: [],
    Description: [],
    Type: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            WarehouseItems.copyOf(warehouseItemsRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "WarehouseItemsUpdateForm")}
      {...rest}
    >
      <TextField
        label="Category"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || warehouseItems)}
